...
function resizeElementMap() {
    $('.mapPopupSingleInitiative').each(function() {
    ...
    $('.mapPopupInnerInitiatives').isotope({
        layoutMode: 'packery'
    });
}
...
function resizeElementMapOrg() {
    $('.mapPopupSingleInitiative').css({
        'width': '',
        'height': '',
        'margin': '',
        'line-height': ''
    })
    $('.mapPopupSingleInitiative').removeClass('resized')
    $('.mapPopupSingleInitiative').removeClass('resized')
    $('.mapPopupInnerInitiatives').isotope({
      layoutMode: 'packery'
    });
...
function toggleResizeElement() {
    $(document).on('change', '.showMembers', function() {
        if ($(this).is(":checked")) {
            resizeElement()
            resizeElementMap()
            resizeElementD3()
...
function layoutVertical() {
    ...
    if ($('.grid').find('.explainList').length == 0) {
        ...
        $('.grid').prepend($newItems).isotope('reloadItems').isotope({
    ...
    resizeElementOrg()
    $('.grid').addClass('vertical').isotope({
...
function metaDataDisplay() {
    $(document).on('change', '.showDate', function() {
        $('.grid.row .date').toggle();
        $('.mapDate').toggleClass('showMapDetail');
    });


    $(document).on('change', '.showName', function() {
        if ($(this).is(":checked")) {
...
function toggleThemeElement() {
    $(document).on('change', '.showTheme', function() {
...
function toggleRoleElement() {
    $(document).on('change', '.showRole', function() {

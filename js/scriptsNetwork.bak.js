...
networkJsonN={"nodes":[],"links":[]}

networkJsonMember={"member":[]}
      ...
      step: function(row) {
        ...
        if(i==0){
          ...
          for (i = 2; i <columnTotal; i++) { 
              ...
              networkJsonN['links'].push({"source":i,"target":i, "value":0});
              networkJsonN['nodes'].push({"name":row.data[0][i],"group":i-1});
          ...
          function runColumn(column, institutionsTotal){
            ...
            if(selectedColumn == 0){
              ...
              if(!isPushed){
                ...
                networkJsonN['nodes'].push({"name":row.data[0][1],"group":column-1});
                isPushed = true
                ...
                networkJsonMember['member'].push({"name":row.data[0][1],"linksTo":institutionsTotalLoop});


              }

            networkJsonN['links'].push({"source":parseInt(row.data[0][0])+institutionsTotal,"target":selectedColumn, "value":1});
...
// }
  ...
  var g = onEnter
      .append("g")
      .attr("class", "node")
      .attr('role', function(d,i) { 
          acroNetwork = networkJsonN.nodes[i].name
          // console.log(acroNetwork)
          res = JSON.search( dataList, '//*[ acro="'+acroNetwork+'" ]' );
          if (res.length>0){
            return res[0].role
          }
      })
      .attr('data-members', function(d,i) { 
          acroNetwork = networkJsonN.nodes[i].name
          // console.log(acroNetwork)
          res = JSON.search( dataList, '//*[ acro="'+acroNetwork+'" ]' );
          if (res.length>0){

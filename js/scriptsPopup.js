...
$(document).on('click', '.membersWrapperLoad', function() {

    $('.popUpWrapper').append('<div class="membersWrapper"><p class="smalltext">Member information:</p></div>')
    $('.membersWrapper').append('<p class="smalltext multipleMember"><b>Members with several connections:</b><br></p>')
    $('.membersWrapper').append('<p class="smalltext singleMember"><b>Members only connecting to ' + $(this).attr('data-savedacro') + ':</b><br></p>')

    popUpNetwork($(this).attr('data-savedacro'))

    $('.membersWrapperLoad').remove()
})
